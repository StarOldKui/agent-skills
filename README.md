...
```markdown
Use codex-rate-limit-reset-credits whenever checking Codex reset credits. The final success response should be only the Markdown table for status, title, granted_at local time, and expires_at local time; do not wrap it in a code fence, and do not print tokens, cookies, raw API responses, or complete unique IDs.
```
